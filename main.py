...
class Live:
    def __init__(self):
        ...
        # Кнопки меню
        self.menu_items = [f'♻️ Еще {LIST_STEP}', 'Отправить геолокацию', 'Выбрать категорию', 'Выбрать подкатегорию',
                           'Менеджер меток']
    ...
    def go_start(self, bot, message, is_start=True):
        ...
        if is_start:
            menu_message = f"Объявлений опубликовано: {active}\n" \
                           f"🔎 Для поиска мест в указанной вами категории отправьте свою геопозицию нажав на кнопку" \
        ...
        def message_text(message):
            ...
            if message.text == self.menu_labels[1]:
                wtf_label = "Метка ✳️ - это точка на карте, в которой происходит производство или реализация" \
                            ...
                            " Что бы разместить метку следует прислать свою геопозицию нажав на кнопку" \
