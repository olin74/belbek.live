...
class Live:
    def __init__(self):
        ...
        # Кнопки меню
        self.menu_items = [f'♻️ Еще {LIST_STEP}', '🔎 Отправить геолокацию 🔍', 'Выбрать категорию',
                           'Выбрать подкатегорию', 'Менеджер меток']
        self.menu_labels = ['Выход', "Что такое метка?", "Мои метки", "✳️ Отправить геолокацию ✳️"]
    ...
    def go_start(self, bot, message, is_start=True):
        ...
        if is_start:
            menu_message = f"Объявлений опубликовано: {active}\n" \
                           f"🔎 Для поиска мест в указанной вами категории отправьте свою геолокацию нажав на кнопку" \
                           f" или прислав координаты текстом.\n" \
    ...
    def go_about(self, bot, message, label_id):
        ...
        bot.send_message(message.chat.id, f"Введите название и краткое описание места (не больше {ABOUT_LIMIT}"
                                          ...
                                          f"\nОно будет выводиться в списке поиска, подробное описание можно будет"
                                          f" добавить позже. Для отмены наберите /cancel",
    ...
    def go_description(self, bot, message, label_id):
        ...
        bot.send_message(message.chat.id, f"📝 Введите подробное описание места "
                                          f"(не больше {DESCRIPTION_LIMIT} символов). Для отмены наберите /cancel",
                         reply_markup=keyboard)
        return
    ...
    def send_full_label(self, bot, message, label_id, is_edit=False):
        ...
        if int(self.users['status'][user_id]) < 0 or int(self.labels['author'][label_id]) != user_id:
            ...
            if int(self.labels['status_label'][label_id]) <= 0:
                ...
                if len(cross) == 0:
                    button_list.append(types.InlineKeyboardButton(text="✳️ Опубликовать ✳️",
                                                                  callback_data=f"pub_{label_id}"))
                else:
